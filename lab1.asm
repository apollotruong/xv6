...
int exitWait(void) {
 ...
 154:	e8 a9 04 00 00       	call   602 <exit>
    ...
    {  
	  printf(2, "\nError using fork\n");
 159:	c7 44 24 04 e0 0c 00 	movl   $0xce0,0x4(%esp)
 160:	00 
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 6e9:	0f b6 92 fa 0c 00 00 	movzbl 0xcfa(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 87f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 881:	b8 f3 0c 00 00       	mov    $0xcf3,%eax
...
free(void *ap)
{
 ...
 941:	a1 c8 0f 00 00       	mov    0xfc8,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 987:	89 10                	mov    %edx,(%eax)
  freep = p;
 989:	a3 c8 0f 00 00       	mov    %eax,0xfc8
}
 98e:	5b                   	pop    %ebx
 98f:	5e                   	pop    %esi
 990:	5f                   	pop    %edi
 ...
 9af:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 9b2:	a3 c8 0f 00 00       	mov    %eax,0xfc8
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 9d9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 9dc:	8b 1d c8 0f 00 00    	mov    0xfc8,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 a19:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 a1b:	3b 15 c8 0f 00 00    	cmp    0xfc8,%edx
...
morecore(uint nu)
{
 ...
 a4d:	e8 ee fe ff ff       	call   940 <free>
  return freep;
 a52:	8b 15 c8 0f 00 00    	mov    0xfc8,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 a74:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 a77:	89 15 c8 0f 00 00    	mov    %edx,0xfc8
      return (void*)(p + 1);
 ...
 a8c:	eb e9                	jmp    a77 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 a8e:	c7 05 c8 0f 00 00 cc 	movl   $0xfcc,0xfc8
 a95:	0f 00 00 
    base.s.size = 0;
 a98:	ba cc 0f 00 00       	mov    $0xfcc,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 a9d:	c7 05 cc 0f 00 00 cc 	movl   $0xfcc,0xfcc
 aa4:	0f 00 00 
    base.s.size = 0;
 aa7:	c7 05 d0 0f 00 00 00 	movl   $0x0,0xfd0
 aae:	00 00 00 
